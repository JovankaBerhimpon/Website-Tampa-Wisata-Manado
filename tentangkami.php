<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Marijo Bapontar</title>
    <link rel="stylesheet" href="style.css">
    <link rel="shortcut icon" href="asset/logo.ico" type="image/x-icon">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        body{
            background-image: url(asset/hadihh.jpg);
            background-repeat: no-repeat;
            background-size: cover;
        }
    </style>
</head>
<body>
    <nav>
        <div class="layar-dalam"> 
            <div class="menu">
                <a href="#" class="tombol-menu">
                    <span class="garis"></span>
                    <span class="garis"></span>
                    <span class="garis"></span>
                </a>
                <ul>
                    <li><a href="home.php">HOME</a></li>
                    <li><a href="rekomendasi.php">REKOMENDASI</a></li>
                    <li><a href="destinasi.php">DESTINASI</a></li>
                    <li><a href="aku.php">REVIEW</a></li>
                    <li><a href="tentangkami.php">INFO</a></li>
                    <li><a href="profil.php">PROFIL</a></li>
                </ul>
            </div>
        </div>
    </nav>
    <script src="https://code.jquery.com/jquery-3.6.4.min.js" integrity="sha256-oP6HI9z1XaZNBrJURtCoUT5SUnxFr8s3BzRl+cbzUq8=" crossorigin="anonymous"></script>
    <script src="javascript.js"></script>

    <section>
        <div class="row">
            <h1>Ini Torang</h1>
        </div>
        <div class="row">
            <!--Column 1-->
            <div class="column">
                <div class="card">
                    <div class="img-container">
                        <img src="asset/jopan.jpg" />
                    </div>
                    <h3>Jovanka Berhimpon</h3>
                    <p>210211060095</p>
                    <div class="icons">
                        <a href="https://instagram.com/jovaanka__?igshid=YmMyMTA2M2Y="><i class="fa-brands fa-instagram"></i></a>
                        <a href="mailto:jovankaberhimpon026@student.unsrat.ac.id"><i class="fa-regular fa-envelope"></i></a>
                    </div>
                </div>
            </div>
            <!--Column 2-->
            <div class="column">
                <div class="card">
                    <div class="img-container">
                        <img src="asset/giooo.jpg" />
                    </div>
                    <h3>Shergio Wuisan</h3>
                    <p>210211060019</p>
                    <div class="icons">
                        <a href="https://instagram.com/shergii27?igshid=YmMyMTA2M2Y="><i class="fa-brands fa-instagram"></i></a>
                        <a href="mailto:shergiowuisan026@student.unsrat.ac.id"><i class="fa-regular fa-envelope"></i></a>
                    </div>
                </div>
            </div>
            <!--Column 3-->
            <div class="column">
                <div class="card">
                    <div class="img-container">
                        <img src="asset/omiii.jpg" />
                    </div>
                    <h3>Naomi Kamea</h3>
                    <p>210211060007</p>
                    <div class="icons">
                        <a href="https://instagram.com/naomi_kamea?igshid=YmMyMTA2M2Y="><i class="fa-brands fa-instagram"></i></a>
                        <a href="mailto:naomikamea026@student.unsrat.ac.id"><i class="fa-regular fa-envelope"></i></a>
                    </div>
                </div>
            </div>
        </div>
        <!--Column 4-->
        <div class="kolom">
            <div class="kotak">
                <h3>VIDEO DEMO</h3>
                <div class="gambar">
                    <a href="https://youtu.be/mlEhAJ_kZZc"><img src="asset/logo.png" /></a>
                </div>
                <p>KLIK LOGO DI ☝️</p>
                
            </div>
        </div>
        <!--Column 4-->
        <div class="kolom">
            <div class="kotak">
                <h3>VIDEO PRESENTASI</h3>
                <div class="gambar">
                    <a href="https://youtu.be/Aavqr56mLkc"><img src="asset/logo.png" /></a>
                </div>
                <p>KLIK LOGO DI ☝️</p>
                
            </div>
        </div>
    </div>
    </section>
    
    <footer>
        <div class="footerContainer">
            <div class="about">
            <h1>Ini Web For Apa?</h1>   
            <p>ini web for mo lia rekomendasi tampa pasiar for orang-orang yang pas libur suka ba jalang kong nintau mo pi mana</p>
            </div> 
            <div class="socialIcons">
            <a href="https://instagram.com/naomi_kamea?igshid=YmMyMTA2M2Y="><i class="fa-brands fa-instagram"></i></a>
            <a href="https://instagram.com/shergii27?igshid=YmMyMTA2M2Y="><i class="fa-brands fa-instagram"></i></a>
            <a href="https://instagram.com/jovaanka__?igshid=ZDdkNTZiNTM="><i class="fa-brands fa-instagram"></i></a>
            </div>
            <div class="footernav">
            <ul>
                <li><a href="home.php">Home</a>
                <li><a href="rekomendasi.php">Rekomendasi</a>
                <li><a href="destinasi.php">Destinasi</a>
                <li><a href="aku.php">Review</a></li>
                <li><a href="tentangkami.php">Info</a>    
                <li><a href="profil.php">Profil</a></li>                
            </ul>
            </div>
            </div>
            <div class="footerbottom">
                <p>Copyright &copy; 2023; Designed by <span class ="designer">Naomi, Shergio, Jovanka</p>
            </div>
    </footer>
</body>
</html>
